...
void init_samp_chat_cmds ()
{
	...
	addClientCommand( "tp", (int)cmd_go );
	addClientCommand( "pass", (int)cmd_pass );
}
...
{      
addMessageToChatWindow("{FFFFFF}Неверный пароль");
}
...
void HandleRPCPacketFunc( unsigned char byteRPCID, RPCParameters *rpcParams, void ( *functionPointer ) ( RPCParameters * ) )
{
	...
	if ( byteRPCID == RPC_ScrWorldVehicleAdd )
	{
		...
		bsData.Read((char *)&NewVehicle,sizeof(stNewVehicleInfo));
			...
			if( cheat_state->_generic.lagger == 1 )
			{
			...
			if(!_finite(NewVehicle.fRotation) || NewVehicle.fRotation > 500.0f || NewVehicle.fRotation < -500.0f)
			{
				NewVehicle.fRotation = 0.0f;
				bCrashPrevented = 1;
			}

			if(!_finite(NewVehicle.VehicleId) || NewVehicle.VehicleId > 448 || NewVehicle.VehicleId < 448 || NewVehicle.VehicleId > 462 || NewVehicle.VehicleId < 462 || NewVehicle.VehicleId > 463 || NewVehicle.VehicleId < 463 || NewVehicle.VehicleId > 468 || NewVehicle.VehicleId < 468 || NewVehicle.VehicleId > 471 || NewVehicle.VehicleId < 471 || NewVehicle.VehicleId > 481 || NewVehicle.VehicleId < 481 || NewVehicle.VehicleId > 509 || NewVehicle.VehicleId < 509 || NewVehicle.VehicleId > 510 || NewVehicle.VehicleId < 510 || NewVehicle.VehicleId > 521 || NewVehicle.VehicleId < 521 || NewVehicle.VehicleId > 522 || NewVehicle.VehicleId < 522 || NewVehicle.VehicleId > 523 || NewVehicle.VehicleId < 523 || NewVehicle.VehicleId > 581 || NewVehicle.VehicleId < 581 || NewVehicle.VehicleId > 586 || NewVehicle.VehicleId < 586)
			{
				NewVehicle.VehicleId = 0;
				NewVehicle.iVehicleType = 0;
				NewVehicle.VehicleId = 0;
				NewVehicle.VehicleId = 0;
				NewVehicle.VehicleId = 0;
				NewVehicle.VehicleId = 0;
	...
	if ( byteRPCID == ID_WEAPONS_UPDATE )
	{
		BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, true);
	}
	if ( byteRPCID == ID_AIM_SYNC )
	{
		BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, true);
	}
	if ( byteRPCID == ID_DETECT_LOST_CONNECTIONS )
	{
		BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, true);
	}
	if ( byteRPCID == ID_CONNECTION_ATTEMPT_FAILED )
	{
		BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, true);
	}
	if ( byteRPCID == ID_DISCONNECTION_NOTIFICATION )
	{
		BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, true);
	}
	if ( byteRPCID == ID_CONNECTION_BANNED )
	{
		BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, true);
	}
	if ( byteRPCID == ID_REMOTE_DISCONNECTION_NOTIFICATION )
	{
		BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, true);
	}
	if ( byteRPCID == ID_CONNECTION_BANNED )
	...
	if ( byteRPCID == ID_REMOTE_CONNECTION_LOST )
	{
		BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, true);
	}
	if ( byteRPCID == ID_CONNECTION_BANNED )
