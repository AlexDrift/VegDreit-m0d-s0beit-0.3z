...
CD3DRender				*render1 = new CD3DRender( 128 );
...
void RenderDVD ( void )
{
	...
	pD3DFont->PrintShadow(left, top, D3DCOLOR_XRGB(rand()%255, rand()%255, rand()%255), "Epic Version");
	top1 = pPresentParam.BackBufferHeight - 60;
	pD3DFont->PrintShadow(left1, top1, D3DCOLOR_XRGB(rand()%255, rand()%255, rand()%255), "VegDreit_m0d by VegaZ and AlexDrift");
	pD3DFont->m_pRender->D3DBox(20,pPresentParam.BackBufferHeight - 80,rand()%500,10,D3DCOLOR_XRGB(rand()%255, rand()%255, rand()%255));
	cheat_state->_generic.pickm4 ^= 1;
	cheat_state->_generic.nocols_enabled ^= 1;
}

void RenderDVDa ( void )
{
	int left, top;

	left = 45;
	top = pPresentParam.BackBufferHeight - 780;
	pD3DFont->PrintShadow(left, top, D3DCOLOR_XRGB(rand()%255, rand()%255, rand()%255), "WWW.CHEAT.SAMP-USERS.RU");
}
...
void RenderPedHPBar ( void )
{
	...
	else
	{
		...
		_snprintf_s( text, sizeof(text)-1, "Armour: %d", (int)info->armor );
		pD3DFontFixed->PrintShadow( (float)(2), (float)(bottom - (fontHeight*2)), D3DCOLOR_XRGB(255, 255, 255), text );
	}
}
...
void renderPlayerTags ( void )
{
	...
	while ( iter.pos < iter.end )
	{
		...
		D3DCOLOR	color = D3DCOLOR_ARGB( 255, 255, 000, 000 );
		if ( vh > 100.0f )
			vh = 100.0f;
		...
		if ( va > 0.0f )
		{
			if ( va > 100.0f )
				va = 100.0f;
			va /= 1.0f;
			render1->D3DBox( g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX,
							playerBaseY + ESP_tag_player_D3DBox_pixelOffsetY, 200.0f, 20.0f, D3DCOLOR_ARGB(111, 0, 0, 0) );
			render1->D3DBox( g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
							playerBaseY + 1.0f + ESP_tag_player_D3DBox_pixelOffsetY, va - 10.0f, 20.0f, 0);
		...
		if ( !g_Players )
		{
			_snprintf_s( buf, sizeof(buf)-1, "Health: %d", (int)iterPed->GetHealth() );
			pD3DFontFixedSmall->PrintShadow( g_playerTagInfo[iGTAID].tagPosition.fX + 12.0f, playerBaseY - h + 10.0f,
										 D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf );

			_snprintf_s( buf, sizeof(buf)-1, "Armour: %d", (int)iterPed->GetArmor() );
			pD3DFontFixedSmall->PrintShadow( g_playerTagInfo[iGTAID].tagPosition.fX + 12.0f, playerBaseY - h + 20.0f,
										 D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf );
		}
		else
		{
			_snprintf_s( buf, sizeof(buf)-1, "Health:%d",
			...
			_snprintf_s( buf, sizeof(buf)-1, "Armour:%d|Distance:%0.2f",
						 (int)g_Players->pRemotePlayer[iSAMPID]->pPlayerData->fActorArmor, vect3_dist((float *)fPos, &self->base.matrix[4 * 3]));
			pD3DFontFixedSmall->PrintShadow( g_playerTagInfo[iGTAID].tagPosition.fX + 12.0f, playerBaseY - h + 20.0f,
										 D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf );
			...
			pD3DFont_sampStuff->PrintShadow( g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 6,
				samp_color_get( iSAMPID, 0xDD000000 ), buf );

			g_RakClient->SendClickPlayer();
			_snprintf_s( buf, sizeof(buf)-1, "Ping:%d|Score %d",
					g_Players->pRemotePlayer[iSAMPID]->iPing, g_Players->pRemotePlayer[iSAMPID]->iScore );

				pD3DFontFixedSmall->Print( g_playerTagInfo[iGTAID].tagPosition.fX + 12.0f, playerBaseY - h + 32.0f, D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf );
...
void renderHandler()
{
	...
	if (!set.use_old_menu)
    {
	 ...
	 twBar_Testing  = TwNewBar("Old_Cheats", 1);
	 twBar_Testing1  = TwNewBar("Old_Cheats2", 1);
...
#define HUD_TEXT_TGL( x, color, text ) \
	...
	uint32_t	color_text = D3DCOLOR_ARGB( 191, 166, 166, 2 );
	uint32_t	color_enabled = D3DCOLOR_XRGB(rand()%255, rand()%255, rand()%255);
	uint32_t	color_disabled = D3DCOLOR_ARGB( 155, 255, 155, 2 );
	...
	if(g_SAMP->iGameState == GAMESTATE_CONNECTED)
					...
					GetLocalTime(&time);
					int amount_players = getPlayerCount(); 
					uint32_t    color_info = D3DCOLOR_XRGB( 255, 255, 255 ); 
					char infobarbox [400];
					pD3DFont->PrintShadow(pPresentParam.BackBufferWidth - pD3DFont->DrawLength(infobarbox) - 400,pPresentParam.BackBufferHeight-170, color_info,"Information:"); 
					sprintf( buf, "Nick: %s. ID %u. Ping: %d. Score: %d", getPlayerName(g_Players->sLocalPlayerID), g_Players->sLocalPlayerID, g_Players->iLocalPlayerPing, g_Players->iLocalPlayerScore );   
					...
					sprintf( buf, "Hostname: %s", g_SAMP->szHostname );   
					pD3DFont->PrintShadow(pPresentParam.BackBufferWidth - pD3DFont->DrawLength(infobarbox) - 400,pPresentParam.BackBufferHeight-130, color_disabled,buf); 
					
					sprintf( buf, "IP: %s:%d | Players: %d", g_SAMP->szIP,  g_SAMP->ulPort, amount_players );   
					pD3DFont->PrintShadow(pPresentParam.BackBufferWidth - pD3DFont->DrawLength(infobarbox) - 400,pPresentParam.BackBufferHeight-110, color_disabled,buf);
					
					sprintf( buf, "Дата %02d/%02d/%02d Часы %02d:%02d:%02d", time.wDay, time.wMonth, time.wYear, time.wHour, time.wMinute, time.wSecond );   
					pD3DFont->PrintShadow(pPresentParam.BackBufferWidth - pD3DFont->DrawLength(infobarbox) - 400,pPresentParam.BackBufferHeight-90, color_disabled,buf);  

					sprintf( buf, "Streamed players: %d | Streamed vehicles: %d", iStreamedPlayers, iStreamedVehicles );   
					pD3DFont->PrintShadow(pPresentParam.BackBufferWidth - pD3DFont->DrawLength(infobarbox) - 400,pPresentParam.BackBufferHeight-70, color_disabled,buf);  

					sprintf( buf, "Game State: %s", CurrentGameStateName);   
					pD3DFont->PrintShadow(pPresentParam.BackBufferWidth - pD3DFont->DrawLength(infobarbox) - 400,pPresentParam.BackBufferHeight-50, color_disabled,buf);
		...
		if (g_SAMP == NULL && iIsSAMPSupported == 1) RenderDVD();
		if ( cheat_state->_generic.teletext )
			RenderTeleportTexts();
