...
void cheat_handle_actor_autoaim ( struct actor_info *info, double time_diff )
{
	...
	if ( !set.use_gta_autoaim )
	{
		...
		if ( cheat_state->actor.autoaim && isAimKeyDown )
		{
			...
			CVector vecAimEnd = vecGunMuzzle + (vecAimMult * 1.0f);
...
// keys/buttons input
			...
			if ( !cheat_state->actor.fly_active )
			{
				...
				{
					// start fly animation
		GTAfunc_PerformAnimation("Cop_Ambient", "Coplook_Loop", -1, 1, 1, 0, 0, 0, 1, 0);
					playerFly_lastAnimationStates = anim_Coplook_Loop;
				}
			}
...
// change animation
				...
				|| playerFly_lastKeySpeedState != keySpeedState )
			{
				...
				case speed_none:
					{
						if (playerFly_lastAnimationStates != anim_Dealer_Idle)
						{
							playerFly_lastAnimationStates = anim_Dealer_Idle;
				...
				case speed_decelerate:
					{
						if ( speed < 0.45f )
						{
							if (playerFly_lastAnimationStates != anim_Coplook_Loop)
							{
								playerFly_lastAnimationStates = anim_Coplook_Loop;
								GTAfunc_PerformAnimation("Cop_Ambient", "Coplook_Loop", -1, 1, 1, 0, 0, 0, 1, 0);
							}
...
// keys/buttons input
			...
			if ( !cheat_state->actor.surfer_active )
			{
				...
				{
					// start fly animation
					GTAfunc_PerformAnimation("SHOP", "SHP_Jump_Land ", -1, 1, 1, 0, 0, 0, 1, 0);
					playerFly_lastAnimationStates = SHP_Jump_Land;
				}
			}
...
// change animation
				...
				|| playerFly_lastKeySpeedState != keySpeedState )
			{
				...
				case speed_none:
					{
						if (playerFly_lastAnimationStates != SHP_Jump_Land)
						{
						...
						case strafe_right:
							{
								...
								else if (playerFly_lastAnimationStates != SHP_Jump_Land)
								{
